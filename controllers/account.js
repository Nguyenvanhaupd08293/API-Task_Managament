...
const getUser = async(req, res) => {
    try {
        db.query('SELECT * FROM accounts', (error, data) => {
            if (error) {
...
const getAccount = async(req, res) => {
    ...
    try {
        db.query('SELECT * FROM accounts WHERE id = ?', [userId], (error, results) => {
            if (error) {
                return res.status(500).json({ message: error.message });
            }
...
const createNewUser = async(req, res) => {
    ...
    try {
        ...
        const sql = "INSERT INTO accounts SET username = ?, email = ?, password = ?";
...
const profileAccount = async(req, res) => {
    ...
    jwt.verify(accessToken, process.env.JWT_ACCESS_KEY, async(err, decoded) => {
        ...
        try {
            db.query('SELECT id, username, email FROM accounts WHERE id = ?', [decoded.id], (error, results) => {
                if (error) {
                    return res.status(500).json({ message: error.message });
                }
